...
for i = 2%length(TH)
    ...
    % Ось X (красная)
    quiver3(x0, y0, z0, xAxis(1)*len, xAxis(2)*len, xAxis(3)*len, 'r', 'LineWidth', 5);
    text(x0 + xAxis(1)*len, y0 + xAxis(2)*len, z0 + xAxis(3)*len, 'X', 'Color', 'r');
    
    % Ось Y (зеленая)
    quiver3(x0, y0, z0, yAxis(1)*len, yAxis(2)*len, yAxis(3)*len, 'g', 'LineWidth', 5);
    text(x0 + yAxis(1)*len, y0 + yAxis(2)*len, z0 + yAxis(3)*len, 'Y', 'Color', 'g');
    
    % Ось Z (синяя)
    quiver3(x0, y0, z0, zAxis(1)*len, zAxis(2)*len, zAxis(3)*len, 'b', 'LineWidth', 5);
    text(x0 + zAxis(1)*len, y0 + zAxis(2)*len, z0 + zAxis(3)*len, 'Z', 'Color', 'b');
